...
class Scheduler:
    def __init__(
        ...
        backend: Backend,
        load_gen_mode: Union[str, LoadGenerationModes] = "sync",
        ...
        if not isinstance(load_gen_mode, LoadGenerationModes):
            load_gen_mode = LoadGenerationModes(load_gen_mode)

        if load_gen_mode != LoadGenerationModes.SYNCHRONOUS and load_gen_rate is None:
            raise ValueError(
