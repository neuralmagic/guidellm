...
export const selectMetricsSummaryLineData = createSelector(
  ...
  (benchmarks, sloState) => {
    ...
    const metrics: (keyof BenchmarkMetrics)[] = ['ttft', 'tpot', 'timePerRequest', 'throughput'];
    ...
    metrics.forEach((metric) => {
      ...
      const valuesByPercentile = percentiles.map((p) => {
        const bmValuesAtP = sortedByRPS.map((bm) => {
          const result =
            bm[metric].percentiles.find((percentile) => percentile.percentile === p)?.value || 0;
          return result;
        });
        const interpolateValueAtP = createMonotoneSpline(requestRates, bmValuesAtP);
        const interpolatedValueAtP = formatNumber(interpolateValueAtP(currentRequestRate));
        return { label: p, value: `${interpolatedValueAtP}` } as PercentileItem;
      });
      const interpolatedPercentileValue =
        Number(valuesByPercentile.find((p) => p.label === enforcedPercentile)?.value) || 0;
      metricData[metric] = {
...
export const selectMetricsDetailsLineData = createSelector([selectBenchmarks], (benchmarks) => {
  const sortedByRPS =
  ...
  const props: (keyof BenchmarkMetrics)[] = ['ttft', 'tpot', 'timePerRequest', 'throughput'];
  props.forEach((prop) => {
